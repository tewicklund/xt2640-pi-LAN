...
#PA_PORTS=['COM8']
PA_PORTS=['/dev/ttyUSB0']
trialog_url="http://192.168.99.49:15110/api/broker/battery"
...
def log_pa_response(pa,measurement_number,query_string,log_file):
    ...
    response=req.get(url=trialog_url)
    soc_string=str(response.json()['soc'])


    #build log string
    log_string=str(measurement_number)+','+date_string+','+time_string+','+measurement_string+','+soc_string
    f=open(log_file,"a")
    f.write(log_string)
    f.close()
...
f.write(query_string.removeprefix('READ?,').removesuffix('\n'))
f.write(',SoC\n')
f.close()
